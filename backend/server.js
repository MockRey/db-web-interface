...
app.post('/query', async (req, res) => {
    try {
        const { sql, role } = req.body; // Получаем SQL-запрос из тела запроса
        ...
        const forbiddenTables = ['credentials'];
        const lowercasedSQL = sql.toLowerCase();// Приводим к нижнему регистру

        // Проверяем, содержит ли запрос запрещённые таблицы
        if (role !== 'admin' && forbiddenTables.some(table => lowercasedSQL.includes(table))) {
            return res.status(403).json({ error: 'Доступ к указанной таблице запрещён!' });
        }

        // Проверяем, содержит ли запрос запрещённые команды
        if (role !== 'admin' && forbiddenCommands.some(cmd => normalizedSQL.startsWith(cmd))) {
...
// Эндпоинт для получения списка шаблонов с запросами
app.post('/templates', async (req, res) => { // Переменная req не будет использоваться, но необходима для сохранения порядка аргументов
    const { login } = req.body;
    
    try {
        const result = await pool.query('SELECT * FROM sql_templates WHERE login = $1',
            [login]
        ); // Получаем шаблоны из таблицы sql_templates
...
app.post('/newTemplate', async (req, res) => {
    const { name, sql, login } = req.body; // Записываем в переменные name, sql и login значения из тела запроса, пришедшего от клиента
    ...
    try {
        await pool.query('INSERT INTO sql_templates (name, sql, login) VALUES ($1, $2, $3) RETURNING *', [name, sql, login]); // Выполняем запрос в базу на добавление нового шаблона
